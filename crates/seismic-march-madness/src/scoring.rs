...
mod tests {
    ...
    fn golden_vectors_encoding() {
        ...
        for v in brackets {
            ...
            let expected_bits = parse_bracket_hex(legacy_hex).unwrap();
    ...
    fn golden_vectors_scoring() {
        ...
        for st in scoring_tests {
            ...
            let contract_bracket = reverse_game_bits(bracket);
            let contract_results = reverse_game_bits(results);
            let contract_score = score_bracket(contract_bracket, contract_results);
            let legacy_api_score = score_bracket_legacy(bracket, results);
            assert_eq!(
