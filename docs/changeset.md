...
### 2026-03-16 — Fix bracket picker clearing later-round picks on earlier-round fill

- **Bug fix**: `makePick` called `clearDownstream()` unconditionally, wiping championship and later-round picks when filling in earlier rounds. Now only clears downstream when changing an existing pick (`oldPick !== null`), not on first pick.
