...
function gradientStyle(rank: number) {
  ...
  return {
    backgroundImage: `
      ...
      radial-gradient(ellipse at 80% 70%, rgba(82, 53, 66, ${intensity * 0.8}) 0%, transparent 70%),
      radial-gradient(ellipse at 60% 20%, rgba(166, 146, 77, ${intensity * 0.5}) 0%, transparent 50%)
    `,
  };
}
  ...
  return (
    <Link to={`/bracket/${entry.address}`} className="block group/link">
      <div className="relative overflow-hidden rounded-2xl bg-bg-secondary hover:bg-bg-hover/50 transition-colors">
        ...
        <Card
          size="sm"
          className="z-10 isolate bg-transparent border-border/50"
        >
          <CardContent>
